...
const PLATFORM_CONFIGS = {
  ...
  tiktok: {
    ...
    supportedMedia: ['video'],
    fields: {
      caption: { required: false, placeholder: 'Describe your video...', maxLength: 150 },
      hashtags: { required: false, placeholder: '#fyp #viral #trending' },
      mentions: { required: false, placeholder: '@username' },
      effects: { required: false, placeholder: 'Add effects' },
      sounds: { required: false, placeholder: 'Original sound or select music' },
      duetSettings: { required: false, type: 'boolean', label: 'Allow duets and stitches' }
    }
  }
};
  ...
  const createPost = async (action: 'draft' | 'publish' | 'schedule') => {
    try {
      ...
      if (selectedPlatform === 'youtube') {
        ...
        postPayload.tiktok_content = {
          effects: postData.effects ? postData.effects.split(',').map((e: string) => e.trim()) : [],
          duet_settings: {
            allow_duet: postData.duetSettings || true,
            allow_stitch: postData.duetSettings || true
          }
        };
      }
                  ...
                  {selectedPlatform && selectedPostType && (
                    <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                      <button
                        ...
                        disabled={loading}
                        className="flex items-center space-x-2 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors disabled:opacity-50"
                      >
                        <Save size={16} />
                      ...
                      {postData.scheduledFor ? (
                        <button
                          ...
                          disabled={loading}
                          className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors disabled:opacity-50"
                        >
                          <Clock size={16} />
                        ...
                        <button
                          ...
                          disabled={loading}
                          className="flex items-center space-x-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors disabled:opacity-50"
                        >
                          <Send size={16} />
